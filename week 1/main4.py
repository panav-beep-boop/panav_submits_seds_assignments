...
# READ THIS!!!
    for i in range(NUM_PARTICLES):
        ...
        nx = d1[0] / d
        ny = d1[1] / d
        v_normal = a[0] * nx + a[1] * ny #a dot product, if u would.
        ...
        for x in range(i+1,NUM_PARTICLES):
            ...
            u=r/r1
            srel=np.dot(v2-v1,u)
            j=(1+RESTITUTION)*srel*u*0.5
            if (r1<=2*PARTICLE_RADIUS):
                 offset=2*PARTICLE_RADIUS-r1
                 positions[i]=positions[i]-0.5*offset*u
                 positions[x]=positions[x]+0.5*offset*u #pushing away in opp direction
                 velocities[x]=velocities[x]-j
